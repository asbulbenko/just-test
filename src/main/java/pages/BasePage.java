...
public class BasePage {
    ...
    public List<WebElement> waitUntilAllElementsAppear(List<WebElement> webElements) {
        try {
            ...
            throw new IllegalArgumentException("List of elements not appeared", e);
        }
    }
    ...
    public Boolean waitIsPageUrlContain(String text) {
        try {
            return wait.until(ExpectedConditions.urlContains(text));
        } catch (TimeoutException e) {
            throw new IllegalArgumentException("page doesn't contain url " + text, e);
        }
    }
}
